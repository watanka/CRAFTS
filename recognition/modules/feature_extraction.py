...
class SimpleResNet_FeatureExtractor(nn.Module) :
    '''A simplified ResNet feature extraction module in CRAFTS
    https://arxiv.org/pdf/2007.09629.pdf
    '''
    def __init__(self, input_channel, num_blocks = [2,5,3], block = SimpleBasicBlock) :
        super(SimpleResNet_FeatureExtractor, self).__init__()
        self.ConvNet = SimpleResNet(input_channel = input_channel, block = block, num_blocks = num_blocks)
...
class SimpleResNet(nn.Module) :
    def __init__(self, input_channel, block, num_blocks, init_weights = True) :
        super().__init__()
